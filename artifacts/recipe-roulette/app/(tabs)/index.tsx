...
function RecipeDetailModal({
  ...
  // Whether this recipe has a "Start Cooking" flow at all — drives both
  // whether that button renders, and which of Start Cooking / Add to
  // Grocery List gets the highlighted (primary-filled) treatment below.
  const hasSteps = (recipe?.instructions.length ?? 0) > 0;
          ...
          <View style={styles.modalBody}>
            ...
            >
              <Feather
                ...
                size={16}
                color={addedToGrocery ? colors.foreground : hasSteps ? colors.primary : colors.primaryForeground}
              />
              <Text
                style={[
            ...
            <Pressable
              ...
              style={({ pressed }) => [
                styles.groceryBtn,
                { backgroundColor: colors.card, borderWidth: 1.5, borderColor: saved ? colors.primary : colors.border },
                pressed && { opacity: 0.9 },
              ]}
            >
              <Feather name="bookmark" size={16} color={saved ? colors.primary : colors.foreground} />
              <Text style={[styles.groceryBtnText, { color: saved ? colors.primary : colors.foreground }]}>
                {saved ? "Saved to My Dinners" : "Save to My Dinners"}
              </Text>
            </Pressable>
...
export default function SpinScreen() {
  ...
  // plan.tsx (fixed to do the same). Without it, this hook instance's
